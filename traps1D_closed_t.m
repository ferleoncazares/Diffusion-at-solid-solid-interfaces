...
% H:        Total amount of solute in the sample [mol]
 ...
 %% Function
function [xr,x,t,ts,clr,cl,civ,Js1,Js2,Ji,H] = traps1D_closed_t(xs,xi,T0,dx,dt,tend,S1,D01,Q1,Eb1,S2,D02,Q2,Eb2,St,DeltaE,DeltaEt,D0t,a,c1_ini,c2_ini,coord,l,nsvs)
...
HIv(1) = HI;
cljr = clj;                             % clj reduced to ignore both boundaries
Jx_l = zeros(size(clj,1)+1,1);          % Flux at each r_l element boundary (Jr_l(1)=0 always due to symmetry)
jss = max(js(1),2);
ji = 1;
...
for j = 1:(length(t)-1)                 % T loop
    ...
    %%% Fluxes
    Jx_l(2:end-1) = -D_T(1:end-1) .* (clj(2:end) - clj(1:end-1))/dx;          % Fick's 1st law - the result does not apply at the boundaries of element I
    ...
    Jx0 = Jx_l(1:end-1).*Ax_l(1:end) - Jx_l(2:end).*Ax_r(1:end);        % Flux into each element * the boundary area [mol/s]
    
    % Fick's 2nd law    
    cljr = Jx0./Vx * dt + cljr;    % dcl/dt
    clj = cljr;                    % Updating cl
